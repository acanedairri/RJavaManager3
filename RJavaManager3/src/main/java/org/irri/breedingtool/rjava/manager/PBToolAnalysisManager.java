...
//						}
	...
	@Override
		public void doSingleEnvironmentAnalysis2(String dataFileName, String outFileName, String resultFolderPath, int designIndex, String[] respvar, 
				String environment, String[] environmentLevels, String genotype, String block, String rep, String row, String column, boolean descriptiveStat, boolean varianceComponents, 
				boolean boxplotRawData, boolean histogramRawData, boolean heatmapResiduals, String heatmapRow, String heatmapColumn, boolean diagnosticPlot, 
	...
	//								rEngine.eval(outspace);
									...
									if (compareControl || specifiedContrast) {
										...
										if (compareControl) {
											...
											rEngine.eval(compareCtrl);
											
											String runSuccessCompareCtrl = rEngine.eval("class(cntrl)").asString();
										...
										if (specifiedContrast) {
											...
											rEngine.eval(readContrastData);
											String runSuccessContrastData = rEngine.eval("contrastData").asString();
											...
											if (runSuccessContrastData != null && runSuccessContrastData.equals("notRun")) {	
												...
												rEngine.eval(userContrast);
												
												String runSuccessUserCtrl = rEngine.eval("class(userContrast)").asString();
												if (runSuccessUserCtrl != null && runSuccessUserCtrl.equals("try-error")) {
													...
													rEngine.eval(checkUError);
													rEngine.eval(checkUError2);
													rEngine.eval(checkUError3);
													rEngine.eval(checkUError4);
...
//									}
	...
	@Override
		public void doMultiEnvironmentOneStage(String dataFileName, String outFileName, String resultFolderPath, int designIndex, 
				String[] respvar, String environment, String[] environmentLevels, String genotype, String block, String rep, String row, String column, 
				boolean descriptiveStat, boolean varianceComponents, boolean boxplotRawData, boolean histogramRawData, boolean diagnosticPlot, 
				boolean genotypeFixed, boolean performPairwise, String pairwiseAlpha, String[] genotypeLevels, 
				String[] controlLevels, boolean compareControl, boolean performAllPairwise, boolean genotypeRandom, 
				boolean stabilityFinlay, boolean stabilityShukla, boolean specifiedContrastGeno, String contrastGenoFilename,
	...
	//									rEngine.rniAssign("controlLevels",rEngine.rniPutStringArray(controlLevels),0); // a string array from OptionsPage
								...
								if (specifiedContrastGeno) {
									...
									System.out.println(readContrastData);
									rEngine.eval(readContrastData);
									String runSuccessContrastData = rEngine.eval("contrastGenoData").asString();
...
//									System.out.println(readContrastData);
//									rEngine.eval(readContrastData);
//									String runSuccessContrastData = rEngine.eval("contrastEnvData").asString();
//									
//									if (runSuccessContrastData != null && runSuccessContrastData.equals("notRun")) {	
//										System.out.println("error");
//										rEngine.eval("capture.output(cat(\"\n***Error reading contrast data.***\n\"),file=\"" + outFileName + "\",append = FALSE)"); 
...
//								} // end stmt if (specifiedContrastEnv)
	...
	// end stability												
								...
								if (gge) {
									if (environmentLevels.length > 2) {
										...
										String setWd="setwd(\"" + resultFolderPath + "\")";
										String ggeOut = "ggeOut <- try(gge.analysis(genoEnvMeans[,match(\"CodedEnv\", names(genoEnvMeans))], genoEnvMeans[,match(\"CodedGeno\", names(genoEnvMeans))], meaOne1$output[[" + i + "]]$harmonicMean, genoEnvMeans[,match(\"" + ybarName + "\", names(genoEnvMeans))], meaOne1$output[[" + i + "]]$MSE, number = FALSE, yVar = \"" + ybarName +"\", f=0.5, graphSym = TRUE, graphEnv = FALSE, graphGeno = FALSE), silent=TRUE)"; 
										String outAmmi1 = "capture.output(cat(\"\nGGE ANALYSIS:\n\"),file=\"" + outFileName + "\",append = TRUE)";
										String outAmmi2 = "capture.output(cat(\"Percentage of Total Variation Accounted for by the Principal Components: \n\n\"),file=\"" + outFileName + "\",append = TRUE)";;
										rEngine.eval(setWd);
										rEngine.eval(ggeOut);
										...
										//f=0
										String ggeOut2 = "ggeOut2 <- try(gge.analysis(genoEnvMeans[,match(\"CodedEnv\", names(genoEnvMeans))], genoEnvMeans[,match(\"CodedGeno\", names(genoEnvMeans))], meaOne1$output[[" + i + "]]$harmonicMean, genoEnvMeans[,match(\"" + ybarName + "\", names(genoEnvMeans))], meaOne1$output[[" + i + "]]$MSE, number = FALSE, yVar = \"" + ybarName +"\", f=0, graphSym = FALSE, graphEnv = TRUE, graphGeno = FALSE), silent=TRUE)"; 
										rEngine.eval(ggeOut2);
										System.out.println(ggeOut2);
										...
										//f=1
										String ggeOut3 = "ggeOut3 <- try(gge.analysis(genoEnvMeans[,match(\"CodedEnv\", names(genoEnvMeans))], genoEnvMeans[,match(\"CodedGeno\", names(genoEnvMeans))], meaOne1$output[[" + i + "]]$harmonicMean, genoEnvMeans[,match(\"" + ybarName + "\", names(genoEnvMeans))], meaOne1$output[[" + i + "]]$MSE, number = FALSE, yVar = \"" + ybarName +"\", f=1, graphSym = FALSE, graphEnv = FALSE, graphGeno = TRUE), silent=TRUE)"; 
										rEngine.eval(ggeOut3);
										System.out.println(ggeOut3);
	...
	@Override
		public void doSingleEnvironmentAnalysisPRep(String dataFileName, String resultFolderPath, String[] respvar, 
				String genotype, String row, String column, String environment, boolean genotypeFixed, 
				boolean genotypeRandom, boolean excludeControls, String[] controlLevel, boolean moransTest, 
				String[] spatialStruc, boolean descriptiveStat, boolean varianceComponents, 
				boolean boxplotRawData, boolean histogramRawData, 
				boolean heatmapResiduals, boolean diagnosticPlot){
			...
			rEngine.eval(sinkIn);
			rEngine.eval(usedData);
			rEngine.eval(analysisDone);
			rEngine.eval(usedDesign);
			
			if (genotypeFixed) {
				System.out.println(funcSSAPRepFixed);
				rEngine.eval(funcSSAPRepFixed);

				String runSuccessCommand = rEngine.eval("class(resultFixed)").asString();
				if (runSuccessCommand.equals("try-error")) {
					String errorMsg1 = "msg1 <- trimStrings(strsplit(resultFixed, \":\")[[1]])";
					String errorMsg2 = "msg1 <- trimStrings(paste(strsplit(msg1, \"\\n\")[[length(msg1)]], collapse = \" \"))";
					String errorMsg3 = "msg1 <- gsub(\"\\\"\", \"\", msg1)";
					...
					rEngine.eval(funcResidFixedWrite);
					
					System.out.println(funcSummaryFixed);
					rEngine.eval(funcSummaryFixed);
					
					System.out.println(funcSummaryFixedWrite);
					rEngine.eval(funcSummaryFixedWrite);
				}
			}
			
			if (genotypeRandom) {
				System.out.println(funcSSAPRepRandom);
				rEngine.eval(funcSSAPRepRandom);
				
				String runSuccessCommand = rEngine.eval("class(resultRandom)").asString();
				if (runSuccessCommand.equals("try-error")) {
					String errorMsg1 = "msg2 <- trimStrings(strsplit(resultRandom, \":\")[[1]])";
					String errorMsg2 = "msg2 <- trimStrings(paste(strsplit(msg2, \"\\n\")[[length(msg2)]], collapse = \" \"))";
					String errorMsg3 = "msg2 <- gsub(\"\\\"\", \"\", msg2)";
					String errorMsg4 = "cat(\"Error in SSATestPrep:\\n\",msg2, sep = \"\")";
					...
					rEngine.eval(funcResidRandomWrite);
					
					System.out.println(funcSummaryRandom);
					rEngine.eval(funcSummaryRandom);
					
					System.out.println(funcSummaryRandomWrite);
					rEngine.eval(funcSummaryRandomWrite);
				}
			}
